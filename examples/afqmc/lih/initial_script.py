...
DEFAULT_CONFIG = RunConfig(
    ...
    pop_control_freq=5,
    max_blocks_cap=128,
    production_blocks=49,
    estimator_window=8,
    estimator_lag=1,
    estimator_decay=0.950095,
    seed=42,
)
...
def run_config(cfg: RunConfig, problem: dict, wall_time_limit: float, target_energy: float) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="afqmc_run_", dir=AFQMC_ROOT) as tmpdir_text:
        ...
        _remaining_budget = max(0.25, wall_time_limit - prep_wall)
        production_blocks = max(1, min(cfg.max_blocks_cap, cfg.production_blocks))
