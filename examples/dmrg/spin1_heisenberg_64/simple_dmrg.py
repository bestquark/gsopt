...
class RunConfig:
    ...
    init_seed: int
    expand_noise: float
    product_even_state: tuple[float, float, float]
    product_odd_state: tuple[float, float, float]
    block_sweep_sequence: str
    local_eig_tol: float
    local_eig_ncv: int
...
MODEL_NAME = "spin1_heisenberg_64"
...
DEFAULT_CONFIG = RunConfig(
    ...
    init_seed=15,
    expand_noise=0.0001,
    product_even_state=(1.0, 0.0, 0.0),
    product_odd_state=(0.0, 0.0, 1.0),
    block_sweep_sequence='RL',
    local_eig_tol=0.001,
    local_eig_ncv=4,
)
...
def normalize_state(vector: tuple[float, float, float]) -> np.ndarray:
...
def run_config(cfg: RunConfig, problem: dict, wall_time_limit: float = 20.0) -> dict:
    ...
    sequence = cfg.block_sweep_sequence or "R"
    solver = qtn.DMRG2(mpo, bond_dims=[cfg.bond_schedule[0]], cutoffs=cfg.cutoff, p0=p0)
    solver.opts["local_eig_tol"] = cfg.local_eig_tol
    solver.opts["local_eig_ncv"] = cfg.local_eig_ncv

    history: list[tuple[int, float, int]] = []
    directions = itertools.cycle(sequence)
    previous_direction = None
    total_sweeps = cfg.max_blocks * len(sequence)
    for sweep in range(total_sweeps):
        if (time.perf_counter() - start) >= wall_time_limit:
            break
        direction = next(directions)
        block = sweep // len(sequence)
        max_bond = cfg.bond_schedule[min(block, len(cfg.bond_schedule) - 1)]
    ...
    if not history:
        ...
        history.append((len(solver.energies), energy, int(solver.state.max_bond())))
