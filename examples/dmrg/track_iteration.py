...
SNAPSHOT_ROOT = Path(os.environ.get("AUTORESEARCH_DMRG_SNAPSHOT_ROOT", SCRIPT_DIR / "snapshots"))
THREAD_BUDGET_ENV = {
    "OMP_NUM_THREADS": "10",
...
def freeze_source_file(source_file: Path) -> tuple[Path, tempfile.TemporaryDirectory[str], bytes, str]:
    ...
    staging_dir = tempfile.TemporaryDirectory(prefix="dmrg_track_", dir=SCRIPT_DIR)
...
def previous_snapshot_file(model: str, iteration: int) -> Path | None:
    for idx in range(iteration - 1, -1, -1):
        candidate = snapshot_dir(model, idx) / "simple_dmrg.py"
        if candidate.exists():
            return candidate
    return None
...
def main():
    parser = argparse.ArgumentParser(description="Run and archive one editable DMRG iteration.")
    ...
    archived_source = out_dir / "simple_dmrg.py"
