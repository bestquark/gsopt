...
DEFAULT_CONFIG = RunConfig(
    ...
    method="dmrg2",
    init_state="neel",
    bond_schedule=(16, 24, 32, 48, 64, 80, 128, 180),
    cutoff=1e-12,
    solver_tol=9.5e-5,
    max_sweeps=32,
    tau=0.1,
...
def build_initial_mps(cfg: RunConfig, problem: dict):
    ...
    if cfg.init_bond_dim > 1:
        ...
        state.expand_bond_dimension(cfg.init_bond_dim, rand_strength=5e-5, create_bond=True)
...
def run_tebd2d(cfg: RunConfig, problem: dict, wall_time_limit: float) -> dict:
    ...
    for step in range(cfg.max_sweeps):
        ...
        tebd.evolve(steps=1, progbar=False)
        history.append((step + 1, float(tebd.energy), max_bond))
        state = tebd.state
    ...
    if not history:
        ...
        tebd.evolve(steps=1, progbar=False)
        history.append((1, float(tebd.energy), cfg.bond_schedule[0]))
        state = tebd.state
...
def run_config(cfg: RunConfig, problem: dict, wall_time_limit: float = 20.0) -> dict:
    ...
    return {
        ...
        "chain_length": problem["lx"] if problem["geometry"] == "1d" else problem["nsites"],
