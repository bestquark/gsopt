...
from track_iteration import model_root, resolve_script, snapshot_source_file
...
def snapshot_file(model: str, iteration: int) -> Path:
    return snapshot_source_file(model_root(model) / f"iter_{iteration:04d}")


def main():
    parser = argparse.ArgumentParser(description="Restore the best archived TN iteration into the live script.")
