"""
Minimal CUDA-Q VQE benchmark for external autoresearch agents.

This file contains the full molecule setup, CUDA-Q kernel definitions,
optimization loop, and scalar evaluation. The active VQE lane is intentionally
just this file plus `search.py`.
"""
...
SUPPORTED_MOLECULES = tuple(MOLECULE_SPECS)
...
def config_signature(cfg: RunConfig) -> tuple:
    return (cfg.ansatz, cfg.layers, cfg.optimizer, cfg.max_steps, cfg.init_scale, cfg.seed)
...
def build_problem(molecule_name: str = "BH") -> dict:
    if molecule_name not in MOLECULE_SPECS:
...
def run_config(
    ...
    x0 = cfg.init_scale * rng.standard_normal(num_params(cfg, problem["n_qubits"]))
    ...
    try:
        result = minimize(
            ...
            method=method_name,
            callback=callback,
            options={"maxiter": cfg.max_steps, "disp": False},
        )
...
def main():
    parser = argparse.ArgumentParser(description="Run the simple CUDA-Q VQE benchmark on one molecule.")
    parser.add_argument("--molecule", default="BH", choices=SUPPORTED_MOLECULES)
    parser.add_argument("--wall-seconds", type=float, default=5.0)
    args = parser.parse_args()

    problem = build_problem(args.molecule)
