...
def freeze_source_file(source_file: Path) -> tuple[Path, tempfile.TemporaryDirectory[str], bytes, str]:
    ...
    staging_dir = tempfile.TemporaryDirectory(prefix="vqe_track_", dir=SCRIPT_DIR)
...
def append_summary(summary_path: Path, row: dict):
    if not summary_path.exists():
        summary_path.write_text(
            "iteration\tfinal_energy\ttarget_energy\tfinal_error\tchem_acc_step\twall_seconds\tsnapshot_dir\n"
        )
    with summary_path.open("a") as handle:
        chem_acc_step = "" if row["chem_acc_step"] is None else row["chem_acc_step"]
        handle.write(
            f"{row['iteration']}\t{row['final_energy']:.12f}\t{row['target_energy']:.12f}\t"
            f"{row['final_error']:.12e}\t{chem_acc_step}\t{row['wall_seconds']:.4f}\t{row['snapshot_dir']}\n"
        )
...
def append_results(results_file: Path, row: dict):
    if not results_file.exists():
        results_file.write_text(
            "iteration\tabs_final_error\tfinal_energy\ttarget_energy\tfinal_error\tchem_acc_step\twall_seconds\tstatus\tdescription\tsnapshot_dir\n"
        )
    with results_file.open("a") as handle:
        ...
        target_energy = "" if row["target_energy"] is None else f"{row['target_energy']:.12f}"
        final_error = "" if row["final_error"] is None else f"{row['final_error']:.12e}"
        description = row["description"].replace("\t", " ").strip()
        snapshot_dir_text = str(row["snapshot_dir"])
        handle.write(
            f"{row['iteration']}\t{abs_final_error}\t{final_energy}\t{target_energy}\t{final_error}\t"
            f"{chem_acc_step}\t{row['wall_seconds']:.4f}\t{row['status']}\t{description}\t{snapshot_dir_text}\n"
        )
