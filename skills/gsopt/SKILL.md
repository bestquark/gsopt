...
- Inside a scaffolded run directory, also treat `_user_evaluate.py`, `run_eval.py`, `restore_best.py`, `status.py`, `plot.py`, `watchdog.py`, `campaign.py`, `async_campaign.py`, `tui.py`, and `slurm_campaign.py` as fixed infrastructure unless the user explicitly asks to change GSOpt itself.
...
For long evaluations, prefer the async driver so the agent sleeps during
scoring and wakes as a fresh session for the next mutation:

```bash
python3 async_campaign.py --agent codex --search
python3 async_campaign.py --agent claude
```

The async driver wakes the agent for one mutation, asks it to write
`logs/campaign/pending_mutation.json`, runs the scored evaluation after the
agent exits, restores the best state if needed, and then relaunches a fresh
agent session while mutations remain.

On Slurm clusters, use the self-resubmitting async driver:
...
Each Slurm job performs one async mutation/evaluation step. When the scored
run finishes, the job checks `status.py`; if more mutations remain, it submits
the next `sbatch` job with the same agent and scheduler settings.
...
- `python3 async_campaign.py --agent codex|claude` wakes one agent per mutation and runs scoring while the agent is asleep.
- `python3 tui.py` displays live status, campaign state, async state, Slurm job state, and recent scored iterations in a terminal dashboard.
- `python3 slurm_campaign.py --agent codex|claude ...` writes scheduler state under `logs/campaign/slurm/` and keeps resubmitting async `sbatch` jobs until the target iteration count is reached.
